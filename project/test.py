...
def game2():
    ...
    upgrades = [
        ...
        {"name": "Прочность", "level": loaded_data[5], "price": 50},
    ]
    ...
    while run:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                print([upgrades[i]['level'] for i in range(6)] + [coins])
                reset_database()
                write_to_database([upgrades[i]['level'] for i in range(6)] + [coins])
                run = False
                
            elif event.type == pygame.MOUSEBUTTONDOWN:
                for i, button in enumerate(buttons):
                    if button.is_clicked(event.pos):
                        if upgrades[i]["level"] < 500 and coins >= upgrades[i]["price"]:
                            upgrades[i]["level"] += 1
                            coins -= upgrades[i]["price"]
        ...
        text_surface = font.render(f"Монеты: {coins}", True, (0, 0, 0))
